...
data.on$g.s <- residuals(lm(log(akde) ~ stem + ch + ch2 + cc + cc2 + 
                               twi + twi2 + north + east + 
                               dOpen + dDM + ed,
                             data = data.on))
...
covs.lin <- c("stem", "ch", "cc", "twi", "north", "east", "dOpen", "dDM", "ed", "g.s")
...
# RS structure
# random slopes
f(MRID1, stem, model = "iid", hyper = hyper.list) +
...
M5.form <- case ~ 
  ...
  # STAND
  stem + stem : a.stem +
    stem : a.stem : year.trt : ret + 
    stem : a.stem : year.trt : pil +
  ch + ch : a.ch +
...
# M6 is definitely the best
# M5 may also be competitive
...
saveRDS(M6.fit, "model_tests/on_M6.rds")
saveRDS(M5.fit, "model_tests/on_M5.rds")
