...
covs.lin <- hs.data |> dplyr::select(g.s, cc, twi, vrm, stem, ch, dEdge)

# correlation
cor(covs.lin, method = "pearson") |> round(2)   # nothing over 0.55

# VIF
calc_vif(covs.lin)  # all < 1.8
...
M.form <- case ~ 
  ...
  f(TSPID8, stem, model = "iid", hyper = hyper.list) +
