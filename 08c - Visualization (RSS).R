...
rss_pred_plot(off.dOM, 4, "dOM", "a.dOM", "off")
rss_pred_plot(off.dDM, 3, "dDM", "a.dDM", "off")
rss_pred_plot(off.shdi, 4, "shdi", "a.shdi", "off")

rss_pred_plot(on.stem, 4, "stem", "a.stem", "on")
rss_pred_plot(on.ch, 4, "ch", "a.ch", "on")
rss_pred_plot(on.dOM, 2, "dOM", "a.dOM", "on")
rss_pred_plot(on.dDM, 3, "dDM", "a.dDM", "on")
rss_pred_plot(on.shdi, 2, "shdi", "a.shdi", "on")
...
# ______________________________________________________________________________
# 6. Differences in availability across treatments ----

# conceivably, not the entire range of variables are available by treatment
# if not, the predictions need to reflect this

# additionally, the mean values should be treatment-specific

# ______________________________________________________________________________

# off
ggplot(off.data |> pivot_longer(cols = a.vo:a.shdi)) +
...
# unsurprisingly, stand-level distributions are much different
# for vo, availability is pretty low (duh)

# on
ggplot(on.data |> pivot_longer(cols = a.stem:a.ed)) +
...
# ______________________________________________________________________________
# 7. Predict using treatment-specific means and ranges
# ______________________________________________________________________________

rss_pred_plot_1 <- function (
